...
public abstract class BaseDAOImpl<T> extends HibernateDaoSupport implements BaseDAO<T> {
	...
	@SuppressWarnings("unchecked")
	public List findList2PojoByNativeSQL(String nativeSql,Class claz, Map<String, Type> scalars) throws DAOException {
	...
	public int getCount(String hql){
		...
		List list = this.getHibernateTemplate().find(h);
		if (list == null || list.isEmpty()) {
			count = new Integer(0);
		} else {
			count = new Integer(list.iterator().next().toString());
		}
		return count;
	}
