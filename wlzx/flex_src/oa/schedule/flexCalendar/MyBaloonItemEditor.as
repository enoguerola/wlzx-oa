...
public class MyBaloonItemEditor implements IItemEditor
{
	...
	private var calendar:MyFlexCalendar;
	 ...
	 */
	public function MyBaloonItemEditor(calendar:MyFlexCalendar, doubleClickOnRenderers:Boolean = false, doubleClickOnCalendar:Boolean = false)
	{
	...
	private function addNewItemPopup(event:CalendarMouseEvent):void
	{
		...
		calendarItem =
				new CalendarItem(event.selectedDate, new Date(event.selectedDate.time + preferedTimeForNewEvents),
						ResourceManager.getInstance().getString("MyFlexCalendar", "editor.newItem"));
	...
	{
		editorPanel = new MyBaloonEditorPanel();
